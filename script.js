...
(function(){
  ...
  let wordQueue = [];
  function resetWordQueue() { wordQueue = shuffle(vocabulary.map((_, idx) => idx)); }
  function getNextWordIndex() { if (wordQueue.length === 0) { resetWordQueue(); } return wordQueue.shift(); }
  ...
  resetScoreBtn.addEventListener('click', () => {
    ...
    function createOptions(options) {
      ...
      options.forEach((def, i) => {
        ...
        radio.onchange = () => { submitBtn.disabled = false; resultDiv.textContent = ""; };
    ...
    quizForm.addEventListener("submit", (e) => {
      ...
      if (selected === currentWordData.correct) {
        ...
        resultDiv.innerHTML = `<span style="color:red;">Oops! Correct answer: </span>
                               <span style="color:green;">"${currentWordData.correct}"</span>`;

      }
      updateScoreDisplay();
      submitBtn.disabled = true;
      Array.from(optionsContainer.querySelectorAll("input[type=radio]")).forEach(input => input.disabled = true);
        ...
        spinBtn.disabled = false;
                      ...
                      function shuffleOptions() {
                        document.querySelectorAll('.question').forEach(function (qDiv) {
                          ...
                          if (!optionsDiv) return ; // no options container, skip
...
/* Pattern Tapper  */
 ...
 const maxLevel = 10;
    ...
    showBtn.addEventListener("click", () => {
      pattern = generatePattern(level + 2);  // pattern length grows with level, minimum 3
      flashPattern();
    });
